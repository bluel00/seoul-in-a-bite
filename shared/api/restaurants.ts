...
export const restaurantApi = {
  ...
  async getRestaurantsByCategory(category: string): Promise<FrontendRestaurant[]> {
    const { data, error } = await supabase
      .from('restaurants')
      .select(`
        *,
  ...
  async getRestaurantsByTheme(themeSlug: string): Promise<FrontendRestaurant[]> {
  ...
  async getAllThemes(): Promise<ThemeInfo[]> {
    ...
    const themes = data.map(theme => ({
      id: theme.id,
      title: theme.name,
      imageUrl: theme.image_url || `https://picsum.photos/seed/${theme.slug}/400/300`,
      slug: theme.slug
    }));
  ...
  async getThemeBySlug(slug: string): Promise<ThemeInfo | null> {
    ...
    return {
      id: data.id,
      title: data.name,
      imageUrl: data.image_url || `https://picsum.photos/seed/${data.slug}/400/300`,
      slug: data.slug
    }
  }
}